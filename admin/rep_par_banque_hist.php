...
<body>
<?php
$url = "../controllers/rep_par_banque_hist.php?u=" . $userId . "&e=" . $entrepriseId;
...
?>		
	  <div id="Main" align="center">
		<table id="dg" class="easyui-datagrid"
			...
			url=<?php echo $url; ?> title="HISTORIQUE - RAPPORT PAR BANQUE" iconCls=""
			toolbar="#tb" rownumbers="true" pagination="true" singleSelect="true"
			showFooter="true">

			<thead>
				<tr>
					<th field="BANQUE" style="width: 150px;" sortable="true">BANQUE</th>
					<th field=ADRESSE style="width: 250px;" sortable="true">ADRESSE
						BANQUE</th>
					<th field=SUPERVISEUR style="width: 200px;" sortable="true">SUPERVISEUR</th>
					<th field="BALANCE_TOTALE" styler="cellStyler"
						style="width: 160px;" sortable="true">BALANCE TOTALE</th>
					<th field=TOTAL_FICHES style="width: 150px;" sortable="true">TOTAL
						FICHES</th>
					<th field=MONTANT_VENTE style="width: 150px;" sortable="true">VENTES</th>
					<th field="MONTANT_PERTE" style="width: 150px;" sortable="true">A
						PAYER</th>
					<th field="MONTANT_COMMISSION" style="width: 155px;"
						sortable="true">COMMISSION</th>
					<!-- <th field="DEVISE" style="width: 160px;" sortable="true">DEVISE</th>-->
				</tr>
			</thead>
		</table>
					...
					<table>
						...
						<tr>
							<td><input id="Banque" name="Banque"
								...
								data-options="valueField:'Id',textField:'Nom',url:'<?php echo $url_combobox_banques; ?>', required:false,editable:true,multiple:true,
								
								onSelect: function(rec){
                                 if(rec.Id =='') {
                                    var list = $('#Banque').combobox('getData');
                                    for (let i = 1; i < list.length; i++) {
                                     $('#Banque').combobox('unselect', list[i].Id);
                                    }
                                 } else {
                                   $('#Banque').combobox('unselect', '');
                                 }
                                 }"
								panelHeight="200px" />
							</td>
	...
	var url='';
        function doSearch() {
            $('#dg').datagrid('load', {
            	Banque: $('#Banque').combobox('getValues'),
            ...
            //alert($('#Banque').combobox('getValues'));
        }
	...
	<script>
        function openNewPrintWindow(){
            var url = 'rep_par_banque_print_hist.php?e=<?php echo $entrepriseId;?>&b='+
            $('#Banque').combobox('getValues')+'&t='+
