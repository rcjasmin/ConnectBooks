    ...
    if($count > 0){
        ...
        if($count > 0){
            ...
            if($count > 0){
                ...
                $rs = getQueryResultObject($dbfile,$query);
                if($rs==null){error("CONNECTION_PROBLEM", "Gen on pwoblem koneksyon. SVP verifye internet ou epi voye ticket a ale anko.");}
                $count = $rs->RESULT; // $db->querySingle($query);
                if($count > 0){
                        ...
                        if($count == 0) {
                            ...
                            if($result == null) {
                                ...
                                if($count == 0) {
                                    ...
                                    $q = "SELECT COUNT(*) AS Count,x1.TypeJeu,x1.Boule, x1.Montant AS MontantTotalJoue, x2.Montant AS MontantLimite FROM pari_detail x1
                                ...
                                WHERE x1.PariId='$PariId' AND x1.Montant > x2.Montant LIMIT 1";
                                    ...
                                    if($result == null) {
                                        ...
                                        if($count == 0) {
                                            ...
                                            $message = "Nou paka vann ".$result->Boule." nan " . $gameName . " pou plis pase " . $result->MontantLimite . " nan ticket sa.";
...
function getQueryResultObject($dbfile,$query) {
    ...
    try{
        ...
        $db->beginTransaction();
        $stmt = $db->query($query);
        if(!$stmt) {throw new Exception('Database Exception');}
